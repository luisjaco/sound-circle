...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
            ...
            if (rgFetch.ok) {
                ...
                if (officialRelease?.id) {
                    const recRes = await fetch(
                        `${MB_BASE}/recording?release=${officialRelease.id}&inc=isrcs&limit=50&fmt=json`,
                        { headers: { 'User-Agent': USER_AGENT, Accept: 'application/json' } }
                    );
        ...
        if (tokenData) {
            // Album Image +
            // Song Data (Spotify batch limit: 50 per request)
            await Promise.all([
                getSpotifyData(songs, tokenData), 
                getAlbumData(album, tokenData)
            ])
        }
...
async function getSpotifyData(songs: any, tokenData: any) {

    const validSongIds = songs.map((s: any) => s.spotify_id).filter(Boolean);
    const songBatches = [];
            ...
            if (res.ok) {
                ...
                if (data.tracks) {
                    for (const spTrack of data.tracks) {
                        if (!spTrack) continue;
                        const index = songs.findIndex((s: any) => s.spotify_id === spTrack.id);
                        if (index !== -1) {
                            let updates: any = {};
...
async function getAlbumData(album: any, tokenData: any) {
    let albumImage = null;
