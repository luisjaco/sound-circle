...
export default function AppleMusicLibraryPage() {
    ...
    const TrackRow = ({ track, index }: { track: Track; index: number }) => (
        <div className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors border-b border-white/5 last:border-b-0">
            <span className="text-xs text-neutral-600 font-mono w-6 text-right shrink-0">{index + 1}</span>
            ...
            <span className="text-xs text-neutral-500 font-mono shrink-0">{formatDuration(track.attributes.durationInMillis)}</span>
            <div className="shrink-0"><AddBtn track={track} /></div>
        </div>
    );
    ...
    const ProgressBar = () => batchResolving ? (
        <div className="flex items-center gap-3 p-3 bg-yellow-500/10 rounded-xl border border-yellow-500/20 mb-4">
            ...
            <div className="flex-1 h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                <div className="h-full bg-linear-to-r from-yellow-400 to-orange-400 rounded-full transition-all duration-300" style={{ width: `${batchProgress.total > 0 ? (batchProgress.current / batchProgress.total) * 100 : 0}%` }} />
            </div>
        </div>
    ) : null;
            ...
            <div className="min-h-screen bg-black text-white pb-20">
                <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
                    ...
                    <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight bg-clip-text text-transparent bg-linear-to-r from-red-500 to-pink-500">Apple Music</h1>
            ...
            <div className="min-h-screen bg-black text-white pb-20">
                <div className="max-w-3xl mx-auto px-6 py-10 space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => resolveAndAddBatch(recentTracks)} disabled={batchResolving || recentTracks.length === 0} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-linear-to-r from-red-500 to-pink-500 text-white disabled:opacity-50">
            ...
            <div className="min-h-screen bg-black text-white pb-20">
                <div className="max-w-3xl mx-auto px-6 py-10 space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => resolveAndAddBatch(playlistTracks)} disabled={batchResolving || loadingPlaylistTracks || playlistTracks.length === 0} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-linear-to-r from-red-500 to-pink-500 text-white disabled:opacity-50">
                            <PlusCircle className="w-3.5 h-3.5" /> Add All
                        </button>
                    </div>
                    ...
                    <div className="flex gap-5 items-center">
                        <div className="w-24 h-24 md:w-28 md:h-28 rounded-xl bg-neutral-800 overflow-hidden shrink-0">
                            {art ? (
                                /* eslint-disable-next-line @next/next/no-img-element */
